...
public class RateJDBCDAO implements IRateDAO {
    ...
    public List<RateDTO> get(Integer currencyId) {
        List<RateDTO> rateDTOList = new ArrayList<>();
        try (Connection conn = DatabaseConnectionFactory.getConnection();
             PreparedStatement st = conn.prepareStatement("SELECT date_exchange_rate, currency_id, currency_cost, CAST(is_day_off AS boolean) FROM app.currency_exchange_rate cer JOIN app.weekends w ON cer.date_exchange_rate = w.calendar_date WHERE currency_id = ?;");
        ){
            st.setInt(1,currencyId);
            try (ResultSet rs = st.executeQuery();) {
                while (rs.next()) {
                     rateDTOList.add(new RateDTO(currencyId,rs.getBigDecimal("currency_cost"), rs.getDate("date_exchange_rate").toLocalDate().atStartOfDay(), rs.getBoolean("is_day_off")));
                }
            }
        } catch (SQLException e) {
            throw new RuntimeException(e);
